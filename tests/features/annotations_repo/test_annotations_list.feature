Feature: Annotations repository List service testing
  ...
  @testrail-C4523037
  @DAT-46431
  Scenario Outline: Hybrid ID-based pagination for annotations
    Given Labels in file: "assets_split/annotations_crud/labels.json" are uploaded to test Dataset
    And Item in path "assets_split/annotations_crud/0000000162.jpg" is uploaded to "Dataset"
    And Item is annotated with annotations in file: "assets_split/annotations_crud/0162_annotations.json"

    When I list annotations with page_size <size>
    Then I save the last_seen_id of that page

    When I list annotations after last_seen_id with page_size <size>
    Then that annotation’s id should not equal the first page’s id
