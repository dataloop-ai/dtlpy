...
@behave.when(u'I generate plugin by the name of "{plugin_name}" to "{src_path}"')
def step_impl(context, plugin_name, src_path):
    ...
    context.project.plugins.generate(name=plugin_name, src_path=src_path)


@behave.then(u'Plugin local files in "{generated_path}" equal plugin local files in '
             u'"{to_compare_path}"')
