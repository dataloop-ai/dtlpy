...
def step_impl(context):
    context.list = context.dataset.items.list(query=None,
                                              page_offset=0,
                                              page_size=100)
...
def step_impl(context):
    assert type(context.list) == entities.PagedEntities
...
def step_impl(context, size):
    context.list = context.dataset.items.list(query=None, page_offset=0, page_size=int(size))
...
def step_impl(context, offset, size):
    context.list = context.dataset.items.list(query=None, page_offset=int(offset), page_size=int(size))
...
@behave.when(u'I list items with query filename="{query}"')
def step_impl(context, query):
    context.list = context.dataset.items.list(entities.Query()(filename=query))
...
@behave.when(u'I list items with query mimetypes="{query}"')
def step_impl(context, query):
    context.list = context.dataset.items.list(entities.Query()(mimetypes=query))
...
@behave.when(u'I list items with query itemType="{query}"')
def step_impl(context, query):
    context.list = context.dataset.items.list(entities.Query()(itemType=query))
