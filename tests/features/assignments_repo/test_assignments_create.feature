Feature: Assignments repository create method testing
    ...
    Scenario: Create - minimum params
        When I create an Assignment from "task" entity
            | assignment_name=min_params | assignee_id=annotator123@dataloop.ai | items=3 |
        Then I receive an assignment entity
        And Assignment has the correct attributes

    Scenario: Create - maximum params - filters
        When I create an Assignment from "task" entity
            | assignment_name=min_params | assignee_id=annotator123@dataloop.ai |  dataset=auto | project_id=auto | filters={"metadata":{"user.good": 3, "user.bad": 3}} | items=None | status=open | metadata={"key": "value"} |
        Then I receive an assignment entity
        And Assignment has the correct attributes

    Scenario: Create - maximum params - items
        When I create an Assignment from "task" entity
            | assignment_name=min_params | assignee_id=annotator123@dataloop.ai |  dataset=auto | project_id=auto | filters=None | items=3 | status=open | metadata={"key": "value"} |
        Then I receive an assignment entity
        And Assignment has the correct attributes