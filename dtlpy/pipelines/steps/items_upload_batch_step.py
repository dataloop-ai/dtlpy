...
class ItemsUploadBatchStep(pipeline_step.PipelineStep):
    """
    Item upload batch step creator
    """

    def __init__(self, step_dict=None):
        super(ItemsUploadBatchStep, self).__init__()
        self.name = 'ItemsUploadBatch'
        ...
        if step_dict is not None:
            ...
            self.inputs = [{'name': 'dataset', 'from': 'dataset',  'by': 'ref','type': 'object'},
                           {'name': 'filepaths', 'from': 'filepaths',  'by': 'ref','type': 'string'}]
    ...
    def execute(self, pipeline_dict):
        ...
        # run
        outputs = dataset.items.upload_batch(**kwargs)
    ...
    def get_arguments():
        ...
        signature = inspect.signature(repositories.Items.upload_batch)
        f_callargs = {
            k: v.default
