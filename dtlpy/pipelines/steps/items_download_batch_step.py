...
class ItemsDownloadBatchStep(pipeline_step.PipelineStep):
    """
    Item download batch step creator
    """

    def __init__(self, step_dict=None):
        super(ItemsDownloadBatchStep, self).__init__()
        self.name = 'ItemsDownloadBatch'
        ...
        if step_dict is not None:
            ...
            self.inputs = [{'name': 'dataset', 'from': 'dataset', 'by': 'ref', 'type': 'object'},
                           {'name': 'items', 'from': 'item_id',  'by': 'ref','type': 'string'}]
    ...
    def execute(self, pipeline_dict):
        ...
        # run
        outputs = dataset.items.download_batch(**kwargs)
    ...
    def get_arguments():
        ...
        signature = inspect.signature(repositories.Items.download_batch)
        f_callargs = {
            k: v.default
