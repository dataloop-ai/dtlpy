...
class PackagesUnpackStep(pipeline_step.PipelineStep):
    """
    Package unpack step
    """

    def __init__(self, step_dict=None):
        super(PackagesUnpackStep, self).__init__()
        self.name = 'PackagesUnpack'
        ...
        if step_dict is not None:
            ...
            self.inputs = [{'name': 'platform_interface', 'from': 'platform_interface', 'type': 'object'},
                           {'name': 'package_id', 'from': 'package_id', 'type': 'string'}]
            self.kwargs = {}
    ...
    def execute(self, pipeline_dict):
        ...
        # run
        outputs = platform_interface.packages.unpack(**kwargs)
