...
class PackagesSessionsUnpackStep(pipeline_step.PipelineStep):
    """
    Package from session unpack step
    """

    def __init__(self, step_dict=None):
        super(PackagesSessionsUnpackStep, self).__init__()
        self.name = 'PackagesSessionsUnpack'
        ...
        if step_dict is not None:
            ...
            self.inputs = [{'name': 'platform_interface', 'from': 'platform_interface', 'type': 'object'},
                           {'name': 'session', 'from': 'session', 'type': 'object'}]
            self.kwargs = {}
    ...
    def execute(self, pipeline_dict):
        ...
        kwargs = self.kwargs
        for input_arg in self.inputs:
            if input_arg['name'] in ['session', 'platform_interface']:
                continue
            # get input item 'name' from 'from'
            kwargs[input_arg['name']] = pipeline_dict[input_arg['from']]

        # run
        outputs = platform_interface.packages.unpack(package_id=session.package, **kwargs)
